<template>
  <view class="news">
      ...
      <view class="news-list" v-for="(item, index) in news" :key="index">
        ...
        <view class="list" v-if="item.list && item.list.length">
          ...
          >
            <view
              class="thumb"
              :style="{ backgroundImage: 'url(' + child.pic + ')' }"
            ></view>
    ...
    <view class="send-box">
      <view class="send-item" v-for="item in contentList">{{ item }}</view>
      <view class="send">
        <input
          class="send-input"
          type="text"
          ...
          v-model="sendcontent"
        />
        <view class="button" @click="send">发送</view>
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  async onShow() {
    wx.cloud
      ...
      .then((res) => {
        console.log(
          "这里是结果",
          res.result.data.map((item) => item.content)
        );
        this.contentList = res.result.data.map((item) => item.content);
      });
  ...
  methods: {
    ...
    input(res) {
      ...
      this.sendcontent = res.detail.value;
    },
    send() {
